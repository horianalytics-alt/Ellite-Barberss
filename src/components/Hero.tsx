...
export function Hero({ booksyUrl }: HeroProps) {
  return (
    <section className="relative min-h-screen flex items-center justify-center pt-24 pb-16 px-4 sm:px-6 lg:px-8 overflow-hidden bg-[#0a0a0a]">
      ...
      {/* Subtle decorative glowing lines */}
      ...
      <div className="relative z-10 max-w-5xl mx-auto text-center flex flex-col items-center">
        ...
        {/* Logo Placeholder (Container for transparent PNG logo) */}
        <div className="mb-6 flex flex-col items-center group">
          <div className="relative w-28 h-28 sm:w-36 sm:h-36 rounded-full border-2 border-[#C9A84C]/60 flex items-center justify-center bg-black/80 backdrop-blur-lg shadow-[0_0_35px_rgba(201,168,76,0.3)] transition-transform duration-500 hover:scale-105">
            {/* Espaço reservado para PNG com fundo transparente do cliente */}
            <div className="flex flex-col items-center justify-center p-2 text-center">
              ...
              <span className="font-serif text-[11px] sm:text-xs tracking-widest text-[#E0C068] font-bold uppercase">
                ELLITE
              </span>
        ...
        <h1 className="font-serif text-4xl sm:text-6xl md:text-7xl font-bold tracking-tight text-white mb-4 max-w-4xl leading-[1.1]">
          Tradição e estilo <br className="hidden sm:inline" />
          <span className="text-gold-gradient italic">no mesmo lugar</span>
        </h1>

        {/* Subtitle */}
        <p className="text-lg sm:text-2xl text-gray-300 font-light tracking-wide max-w-2xl mb-10 leading-relaxed">
          Cortes masculinos e barba em{" "}
          <span className="text-white font-medium">Arujá-SP</span>. A combinação
          perfeita entre técnicas clássicas e tendências modernas.
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto mb-14">
          ...
          <a
            href="#servicos"
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-8 py-4 rounded-full border border-white/20 bg-black/40 backdrop-blur-sm text-gray-200 font-medium text-sm sm:text-base hover:border-[#C9A84C] hover:text-[#C9A84C] hover:bg-black/80 transition-all duration-300"
          >
            Ver Serviços & Preços
          </a>
        </div>

        {/* Key Highlights / Badges Bar */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 w-full max-w-3xl pt-8 border-t border-[#C9A84C]/20">
          <div className="flex items-center justify-center gap-3 p-3 rounded-xl bg-[#121212]/70 border border-white/5">
            ...
            <div className="text-left">
              <p className="text-xs text-gray-400">Segunda a Domingo</p>
              <p className="text-sm font-semibold text-white">09h às 20h</p>
            </div>
          </div>
      ...
      <a
        ...
        className="absolute bottom-4 left-1/2 -translate-x-1/2 text-[#C9A84C]/70 hover:text-[#C9A84C] transition-colors animate-bounce"
      >
