      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {sorted.map((item) => (
            <div
              key={item.id}
              className="group relative h-80 rounded-2xl overflow-hidden border border-[#C9A84C]/20 hover:border-[#C9A84C] transition-all duration-500 bg-[#121212] shadow-[0_4px_20px_rgba(0,0,0,0.5)] hover:shadow-[0_0_30px_rgba(201,168,76,0.25)]"
            >
              <img
                src={item.url}
              ...
              {/* Subtle hover badge without filename */}
              <div className="absolute bottom-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-black/70 backdrop-blur-md border border-[#C9A84C]/40 text-[11px] font-semibold text-[#E0C068] tracking-wider uppercase">
                  <Scissors className="w-3 h-3 text-[#C9A84C]" /> Ellite Barberss
                </span>
              </div>
            </div>
          ))}
